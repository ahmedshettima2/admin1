...
if (isset($_GET['id']) && $_GET['id'] != '') {
	...
	$res = mysqli_query($con, "select * from vendor where id='$id'");
...
if (isset($_POST['submit'])) {
	...
	$res = mysqli_query($con, "select * from vendor where name='$username'");
	$check = mysqli_num_rows($res);
	if ($check > 0) {
		if (isset($_GET['id']) && $_GET['id'] != '') {
	...
	if ($msg == '') {
		if (isset($_GET['id']) && $_GET['id'] != '') {
			$update_sql = "update vendor set name='$username',password='$password',email='$email',phone='$mobile' where id='$id'";
			mysqli_query($con, $update_sql);
		} else {
			mysqli_query($con, "insert into vendor(name,password,email,phone) values('$username','$password','$email','$mobile')");
		}
		header('location:vendor_management.php');
		die();
	}
